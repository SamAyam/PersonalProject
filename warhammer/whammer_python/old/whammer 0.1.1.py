...
class Game_variables():
    def __init__(self, playerkind):
        ...
        global user, main_player, online_players, clientlist, parent, online_ships
...
class Planet():
    def __init__(self, name, size=(12000, 12000), planet_type):
        self.size=size
        self.name=name
        self.spacex=0
        self.spacey=0
...
class checkingclientThread (threading.Thread):
    def __init__(self, clientsock, addr):
        ...
        self.addr=addr
    def run(self):
        global online, socksend, gamex, gamey
##        for guy in npc:
##            send_to_host(["just_one",self.addr,['spawnstatue',guy,guy.x-gamex,guy.y-gamey]])
##        for dude in main_player:
##            for guy in dude.myarmy:
##                send_to_host(["just_one",self.addr,['spawnsoldier',guy,guy.x-gamex,guy.y-gamey]])
##        for guy in main_player:
##            send_to_host(["just_one",self.addr,['spawnplayer',guy,guy.x-gamex,guy.y-gamey]])
##            print("hello")
        while online==True:
            try:
...
def go_online():
    try:
        global socksend, sockrec, port, host, online, parent
        sethost()
...
def translate(message):
    global npc, gamex, gamey, host, port, parent
    if type(message)==str:
        print(message)
        #^^^^delete?
...
##        for guy in npc:
##            send_to_host(["just_one",fromaddr,['spawnstatue',guy,guy.x-gamex,guy.y-gamey]])
##        for dude in main_player:
##            for guy in dude.myarmy:
##                send_to_host(["just_one",fromaddr,['spawnsoldier',guy,guy.x-gamex,guy.y-gamey]])
##        for guy in main_player:
##            send_to_host(["just_one",fromaddr,['spawnplayer',guy,guy.x-gamex,guy.y-gamey]])
##            print("hello")
...
def loading_game():
    ...
    # Later this will load the game title.
    # this will go in the game_variables class:
    global game_map
    game_map=pygame.sprite.Group()
    space1=Space('space1',1)
    first_planet=Planet('planetA','desert')
    space1.planets.add(first_planet)
    game_map.add(space1)
    space2=Space('space2',2)
    space2.planets.add(Planet('planetB','grass'))
    space2.planets.add(Planet('planetC','dying'))
    space2.planets.add(Planet('planetD','dead'))
    game_map.add(space2)
    space3=Space('space3',1)
    space3.planets.add(Planet('planetE','water'))
    space3.planets.add(Planet('planetF','dry'))
    game_map.add(space3)
    space4=Space('space4',3)
    space4.planets.add(Planet('planetG','scorched'))
    space4.planets.add(Planet('planetH','burned'))
    space4.planets.add(Planet('planetI','dead'))
    game_map.add(space4)
    load_planet(first_planet)
    try:
...
def load_space(level):
    ...
    global background, game_map, gamew, gameh
...
def main_game_loop():
    ...
    gvars= Game_variables(playerkind)
